...
var gameState = {
	...
	create: function(){
        ...
        textScore = game.add.text(100, 25, 'Score: 0', { font: "bold 32px Arial", fill: "#D1D1D1", align: "center"});
        textScore.anchor.setTo(0.5, 0.5);
        textScore.setShadow(3, 3, 'rgba(0,0,0,0.5)', 2);

            //Lifes
        textLives = game.add.text(270, 25, 'Lives: x3', { font: "bold 32px Arial", fill: "#D1D1D1", align: "center"});
        textLives.anchor.setTo(0.5, 0.5);
        textLives.setShadow(3, 3, 'rgba(0,0,0,0.5)', 2); 

            //Levels
        textLevel = game.add.text(700, 25, 'Level: 1', { font: "bold 32px Arial", fill: "#D1D1D1", align: "center"});
        textLevel.anchor.setTo(0.5, 0.5);
        textLevel.setShadow(3, 3, 'rgba(0,0,0,0.5)', 2); 

            //Timer
        textTime = game.add.text(520, 25, 'Time: 60', { font: "bold 32px Arial", fill: "#D1D1D1", align: "center"});
		...
		buttonP.events.onInputUp.add(function () {
			...
			choiseLabel = game.add.text(w/2, h-300, 'Click to continue', { font: '30px Arial', fill: '#fff' });
	...
	update: function(){
		...
		game.physics.arcade.collide(alien, bullets, 
		...
		game.physics.arcade.collide(alien, hearts, 
			function (alien, heart) {
				heart.kill();
				upLive++;
				textLives.setText('Lives: x' + upLive);
				fx.play('numkey');
   			});
		game.physics.arcade.collide(alien, coins, 
			function (alien, coin) {
				coin.kill();
				upScore++;
